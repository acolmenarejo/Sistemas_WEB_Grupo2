...
public class pantallaInicial extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(autenticadoObj != null && (boolean) autenticadoObj) {
			...
			out.println("</div></li>");
			out.println("<li class='nav-item'><a class='nav-link' href='/Proyecto_SW1/cerrarSesion'>Cerrar Sesion</a></li>");
			out.println("</ul>");
			out.println("</div>");
			out.println("</nav>");
			...
			try {
				...
				synchronized(statement) {
					rs = statement.executeQuery("SELECT * FROM post ORDER BY id_post DESC LIMIT 20");
				}
				...
				out.close();
				System.out.println("[inicio] cierra out");
			}	
